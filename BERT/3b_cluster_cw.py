...
# Load embeddings
with open(Path("data") / "cw_embeddings_bertbase.json", "r") as f:
    data = json.load(f)

doc_embeddings = np.array(data["embeddings"])
doc_filenames = data["songs"]
...
# ax1.plot(list(k_range), inertias, "bo-", color="#b50b60")
# ax1.set_xlabel("Number of Clusters (k)")
# ax1.set_ylabel("Inertia (within-cluster sum of squares)")
# ax1.set_title("Elbow Method")
# ax1.grid(True, alpha=0.3)

# ax2.plot(list(k_range), silhouettes, "ro-", color="#fc52a7")
...
# plt.tight_layout()
# plt.savefig("cw_cluster_selection.png", dpi=150, bbox_inches="tight")
# plt.show()
...
ax.set_title(f"Song Lyrics Clusters ({method}, k={K})")
ax.legend(loc="upper right", fontsize=8)

plt.tight_layout()
plt.savefig("cw_clusters.png", dpi=150, bbox_inches="tight")
plt.show()

# print(f"\nSaved plots to cw_cluster_selection.png and cw_clusters.png")

print("about to begin cluster output")
...
with open(Path("data") / "cw_cluster_assignments.json", "w") as f:
    json.dump(cluster_output, f, indent=2)

print("Saved cluster assignments to data/cw_cluster_assignments.json")